...
/**
 * Handle forward requests according to the forward algorithm TODO: link for published article
 * 
...
public class ForwardHandler extends AbstractHandler {
	...
	final FindValueOperation findValueOperationProvider, final ForwardRequest forwardRequestProvider, final MessageDispatcher<Void> msgDispatcherProvider,

	final Node localNode, final int myColor, final int nrColors, final int kBucketSize, final long timeout,

	final ExecutorService opExecutor, final ExecutorService forwardExecutor,

	// testing
			final AtomicInteger nrFindNodesWithWrongColor, final AtomicInteger nrForwardHandling, final AtomicInteger nrForwardHandlingFromInitiator, final AtomicInteger nrShortForwardTimeouts, final AtomicInteger nrNacksSent) {
	...
	private void doFindValue(final ForwardRequest req) throws RejectedExecutionException {
		opExecutor.execute(new Runnable() {
			...
			public void run() {
				...
				final FindValueOperation op = findValueOperationProvider.setBootstrap(req.getBootstrap()).setKey(req.getKey());

				final List<Node> results = op.doFindValue();

				// System.out.println(localNode+": finished find node, returning results");

				final ForwardMessage msg = req.generateMessage(localNode).setFindNodeHops(op.getNrQueried()).setPathLength(0).setNodes(results);
	...
	private MessageDispatcher<Void> generateExpectDispatcher(final ForwardRequest incomingReq, final ForwardRequest outgoingReq) {
		...
		return msgDispatcherProvider.setConsumable(true).addFilter(new IdMessageFilter(outgoingReq.getId())).addFilter(new TypeMessageFilter(ForwardMessage.class)).setTimeout(timeout, TimeUnit.MILLISECONDS).setCallback(null, new CompletionHandler<KadMessage, Void>() {
			...
			public void completed(final KadMessage msg, final Void nothing) {
				...
				try {
					kadServer.send(incomingReq.getSrc(), incomingReq.generateMessage(localNode).setFindNodeHops(res.getFindNodeHops()).setPathLength(1 + res.getPathLength()).setNodes(res.getNodes()));
	...
	private void sendForwardRequest(final ForwardRequest incomingReq, final ForwardRequest outgoingReq, final MessageDispatcher<Void> expectDispatcher, final Node nextHop) {

		msgDispatcherProvider.setConsumable(true).addFilter(new IdMessageFilter(outgoingReq.getId())).addFilter(new TypeMessageFilter(ForwardResponse.class)).setCallback(null, new CompletionHandler<KadMessage, Void>() {
			...
			public void completed(final KadMessage msg, final Void nothing) {
				...
				if (res.isAck()) {
					...
					try {
						kadServer.send(incomingReq.getSrc(), incomingReq.generateMessage(localNode).setPathLength(0).setNodes(res.getNodes()));
	...
	/**
	 * finds the next hop according to its color If there is a node with the right color, return it otherwise, if there is a node with a better key than me, return it if there isn't, return null
	 * 
	 * @param bootstraps
	 * @return
	 */
	...
	private void doForwardAndSendAckOrNack(final ForwardRequest incomingReq) {
		...
		final ForwardRequest outgoingReq = forwardRequestProvider.setKey(incomingReq.getKey()).setBootstrap(mergedBootstraps);
