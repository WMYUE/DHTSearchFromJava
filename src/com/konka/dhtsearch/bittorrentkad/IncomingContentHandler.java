...
public class IncomingContentHandler<A> implements CompletionHandler<KadMessage, A> {
	 ...
	 *            the executor for all incoming requests that should be handled by the KeybasedRouting user
	 * @param localNode
	 *            the local node
	 * @param kadServer
	 *            the KadServer used to send messages
	 */
	IncomingContentHandler(ExecutorService clientExecutor, Node localNode, Communicator kadServer) {
	...
	private void handleRequest(final ContentRequest req) {
		...
		clientExecutor.execute(new Runnable() {
			...
			public void run() {
				...
				try {
					kadServer.send(req.getSrc(), req.generateResponse(localNode).setContent(resContent));
				} catch (IOException e) {
